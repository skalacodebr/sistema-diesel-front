...
export default function VeiculoForm({ params }: VeiculoFormProps) {
  ...
  useEffect(() => {
    if (veiculo.modelos_veiculos_id > 0 && modelos.length > 0) {
      ...
      if (modeloSelecionado && modeloSelecionado.marca_veiculo_id !== selectedMarcaId) {
        setSelectedMarcaId(modeloSelecionado.marca_veiculo_id)
      }
    }
  }, [veiculo.modelos_veiculos_id, modelos])

  // Efeito para filtrar modelos quando uma marca é selecionada
  useEffect(() => {
    if (selectedMarcaId && veiculo.modelos_veiculos_id > 0) {
      const modeloAtual = modelos.find(m => m.id === veiculo.modelos_veiculos_id)
      // Se o modelo atual não pertence à marca selecionada, limpar a seleção
      if (modeloAtual && modeloAtual.marca_veiculo_id !== selectedMarcaId) {
  ...
  const fetchModelos = async (marcaId: number) => {
    ...
    try {
      ...
      const data = await getModelosVeiculos(token, marcaId)
      setModelos(data)
    } catch (error) {
